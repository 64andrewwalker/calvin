    ...
    FS: FileSystem,
{
    ...
    fn process_lockfile(
        ...
        for (key, expected_hash) in entries_to_process {
            ...
            if !options.force && !has_calvin_signature(&content) {
                // Skill supplemental files may not include Calvin signature markers (scripts, etc.).
                // If the skill's SKILL.md is Calvin-signed, treat the whole directory as managed.
                if !self.is_part_of_calvin_skill(&path) {
            ...
            if actually_delete {
                ...
                // Best-effort: prune empty skill directories after deleting managed files.
                if let Some(skill_root) = skill_root_from_path(&path) {
    ...
    fn is_part_of_calvin_skill(&self, path: &Path) -> bool {
        ...
        let Ok(content) = self.fs.read(&skill_md) else {
            return false;
        };
...
"#,
    ...
    #[test]
    fn clean_deletes_skill_supplemental_without_signature_when_skill_md_is_signed() {
        let dir = tempdir().unwrap();
        let lockfile_path = dir.path().join("calvin.lock");
        std::fs::create_dir_all(lockfile_path.parent().unwrap()).unwrap();

        // Create a Calvin-managed skill directory.
        let skill_dir = dir.path().join(".claude/skills/my-skill");
        std::fs::create_dir_all(skill_dir.join("scripts")).unwrap();
        std::fs::write(
            skill_dir.join("SKILL.md"),
            "<!-- Generated by Calvin. Source: skills/my-skill/SKILL.md. DO NOT EDIT. -->\n\n# Skill\n",
        )
        .unwrap();

        // Supplemental script without signature.
        let script_path = skill_dir.join("scripts/validate.py");
        let script_content = "print('ok')\n";
        ...
        let normalized_path = normalize_path_for_key(&script_path);
        std::fs::write(
            &lockfile_path,
            format!(
                r#"
...
"#,
        ...
        assert!(
            !script_path.exists(),
            "skill supplemental should be deleted"
        );
        assert!(
            !skill_dir.join("scripts").exists(),
            "empty skill subdirectory should be removed"
        );
    }
