...
//!
//! **Migration Note**: Core types are being migrated to `domain::services::orphan_detector`.
//! For new code, consider using `domain::services::OrphanDetector::detect()`.
...
// Re-export common utilities from domain for consistency
pub use crate::domain::services::{extract_path_from_key, has_calvin_signature, CALVIN_SIGNATURES};
...
pub struct OrphanFile {
    ...
    pub key: String,
    /// The path portion of the key (extracted for convenience)
    pub path: String,
...
// extract_path_from_key is re-exported from domain::services above
...
mod tests {
    ...
    fn test_orphan_is_safe_to_delete() {
        let mut orphan = OrphanFile::from_key("home:~/.claude/commands/test.md");
        orphan.has_signature = true;
        orphan.exists = true;
        assert!(orphan.is_safe_to_delete());

        let mut orphan_no_sig = OrphanFile::from_key("home:~/.claude/commands/test.md");
        orphan_no_sig.has_signature = false;
        orphan_no_sig.exists = true;
        assert!(!orphan_no_sig.is_safe_to_delete());

        let mut orphan_not_exists = OrphanFile::from_key("home:~/.claude/commands/test.md");
        orphan_not_exists.has_signature = true;
        orphan_not_exists.exists = false;
    ...
    fn delete_orphans_deletes_safe_files_only() {
        ...
        let mut orphan_safe = OrphanFile::from_key(&format!("project:{}", safe_file.display()));
        orphan_safe.path = safe_file.to_string_lossy().to_string();
        orphan_safe.has_signature = true;
        orphan_safe.exists = true;

        let mut orphan_unsafe = OrphanFile::from_key(&format!("project:{}", unsafe_file.display()));
        orphan_unsafe.path = unsafe_file.to_string_lossy().to_string();
        orphan_unsafe.has_signature = false;
        orphan_unsafe.exists = true;

        let orphans = vec![orphan_safe, orphan_unsafe];
