...
pub struct DeleteResult {
    ...
    /// Warnings encountered during deletion (e.g., failed to delete)
    ...
    for orphan in orphans {
        ...
        if should_delete {
            ...
            if let Err(e) = fs.remove_file(&expanded) {
                // Collect warning but don't fail - continue with other files
                result
                    .warnings
                    .push(format!("Failed to delete {}: {}", expanded.display(), e));
            } else {
