...
impl<'a> SyncEngine<'a, LocalFileSystem> {
    ...
    pub fn new(
        ...
        Self::new_with_fs(outputs, destination, lockfile_path, options, LocalFileSystem)
    }
...
impl<'a, FS: FileSystem> SyncEngine<'a, FS> {
    ...
    where
        R: crate::sync::conflict::ConflictResolver,
    {
        use crate::sync::conflict::{ConflictChoice, unified_diff};
        ...
        for conflict in conflicts {
            ...
            let reason = match conflict.reason {
                crate::sync::plan::ConflictReason::Modified => crate::sync::conflict::ConflictReason::Modified,
                crate::sync::plan::ConflictReason::Untracked => crate::sync::conflict::ConflictReason::Untracked,
            };
...
mod tests {
    ...
    fn engine_local_skips_unchanged_files() {
        ...
        assert!(result2.written.is_empty(), "Expected no writes on second sync");
    ...
    fn engine_dry_run_does_not_write() {
        ...
        assert!(!dir.path().join("test.md").exists(), "Dry run should not create files");
    }
    ...
    fn engine_with_mock_fs_detects_conflicts() {
        ...
        assert!(plan.to_write.is_empty(), "No files should be in to_write when there's a conflict");
    }
    ...
    impl crate::sync::conflict::ConflictResolver for MockResolver {
        fn resolve_conflict(&mut self, _path: &str, _reason: crate::sync::conflict::ConflictReason) -> crate::sync::conflict::ConflictChoice {
            let choice = self.choices.get(self.call_count)
                .copied()
    ...
    fn engine_interactive_overwrite_with_mock_resolver() {
        ...
        // First sync to create lockfile
        let outputs_v1 = vec![make_output(".claude/test.md", "original content")];
        let options_force = SyncEngineOptions { force: true, ..Default::default() };
        ...
        let outputs_v2 = vec![make_output(".claude/test.md", "new generated content")];
        let options_interactive = SyncEngineOptions { interactive: true, ..Default::default() };
        ...
        let result = engine2.sync_with_resolver(&mut resolver, None::<fn(SyncEvent)>).unwrap();
        
        assert_eq!(resolver.call_count, 1, "Resolver should be called once");
        assert_eq!(result.written.len(), 1, "Should overwrite the file");
        
        // Verify content was overwritten
        let content = mock_fs.read_to_string(Path::new("/mock/root/.claude/test.md")).unwrap();
    ...
    fn engine_interactive_skip_with_mock_resolver() {
        ...
        let engine1 = SyncEngine::new_with_fs(
            ...
            SyncDestination::Local(PathBuf::from("/mock/root")),
            PathBuf::from("/mock/root/.promptpack/.calvin.lock"),
            SyncEngineOptions { force: true, ..Default::default() },
            mock_fs.clone(),
        );
        engine1.sync().unwrap();
        
        // External modification
        mock_fs.write_atomic(Path::new("/mock/root/.claude/test.md"), "user edit").unwrap();
        ...
        let engine2 = SyncEngine::new_with_fs(
            ...
            SyncDestination::Local(PathBuf::from("/mock/root")),
            PathBuf::from("/mock/root/.promptpack/.calvin.lock"),
            SyncEngineOptions { interactive: true, ..Default::default() },
            mock_fs.clone(),
        );
    ...
    fn engine_interactive_diff_then_overwrite() {
        ...
        // Modify
        mock_fs.write_atomic(Path::new("/mock/root/.claude/test.md"), "modified").unwrap();
        ...
        assert!(resolver.diffs_shown[0].contains("-modified"), "Diff should show removed line");
        assert!(resolver.diffs_shown[0].contains("+new"), "Diff should show added line");
        assert_eq!(result.written.len(), 1);
    }
    ...
    fn engine_interactive_overwrite_all() {
        ...
        // Modify both
        mock_fs.write_atomic(Path::new("/mock/root/.claude/a.md"), "a user").unwrap();
        mock_fs.write_atomic(Path::new("/mock/root/.claude/b.md"), "b user").unwrap();
        ...
        assert_eq!(resolver.call_count, 1, "Resolver should only be called once");
        assert_eq!(result.written.len(), 2, "Both files should be written");
        
        // Verify both overwritten
        assert_eq!(mock_fs.read_to_string(Path::new("/mock/root/.claude/a.md")).unwrap(), "a new");
        assert_eq!(mock_fs.read_to_string(Path::new("/mock/root/.claude/b.md")).unwrap(), "b new");
    }
    ...
    fn engine_sync_with_empty_content() {
        ...
        let options = SyncEngineOptions { force: true, ..Default::default() };
        let engine = SyncEngine::local(&outputs, dir.path().to_path_buf(), options);
    ...
    fn engine_sync_with_special_path_characters() {
        ...
        let outputs = vec![make_output(".claude/path with spaces/test-file_v2.md", "content")];
        let options = SyncEngineOptions { force: true, ..Default::default() };
        let engine = SyncEngine::local(&outputs, dir.path().to_path_buf(), options);
        ...
        assert!(dir.path().join(".claude/path with spaces/test-file_v2.md").exists());
    }
    ...
    fn engine_sync_with_preexisting_lockfile() {
        ...
        fs::write(lockfile_dir.join(".calvin.lock"), r#"
version = 1
...
"#).unwrap();
    ...
    fn engine_sync_skip_mode_leaves_conflicts_unchanged() {
        ...
        // External modification
        mock_fs.write_atomic(Path::new("/mock/root/.claude/test.md"), "modified by user").unwrap();
    ...
    fn engine_sync_non_interactive_skips_conflicts_silently() {
        ...
        let engine2 = SyncEngine::new_with_fs(
            ...
            SyncDestination::Local(PathBuf::from("/mock/root")),
            PathBuf::from("/mock/root/.promptpack/.calvin.lock"),
            SyncEngineOptions { interactive: false, force: false, ..Default::default() },
            mock_fs.clone(),
        );
    ...
    fn engine_dry_run_does_not_create_files() {
        ...
        let options = SyncEngineOptions { dry_run: true, force: true, ..Default::default() };
        let engine = SyncEngine::local(&outputs, dir.path().to_path_buf(), options);
    ...
    fn engine_sync_callback_receives_correct_events() {
        ...
        let result = engine.sync_with_callback(Some(|e: SyncEvent| {
            events.push(e);
        })).unwrap();
        ...
        let start_count = events.iter().filter(|e| matches!(e, SyncEvent::ItemStart { .. })).count();
        let written_count = events.iter().filter(|e| matches!(e, SyncEvent::ItemWritten { .. })).count();
