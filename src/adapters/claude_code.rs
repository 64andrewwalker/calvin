...
impl TargetAdapter for ClaudeCodeAdapter {
    ...
    fn compile(&self, asset: &PromptAsset) -> CalvinResult<Vec<OutputFile>> {
        ...
        let command_path = PathBuf::from(".claude/commands").join(format!("{}.md", asset.id));
...
mod tests {
    ...
    fn test_claude_adapter_compile_basic() {
        ...
        assert_eq!(
            outputs[0].path,
            PathBuf::from(".claude/commands/test-command.md")
        );
        ...
        assert!(
            outputs[0].content.starts_with("Test command"),
    ...
    fn test_claude_adapter_compile_no_description_uses_h1() {
        ...
        assert!(
            outputs[0].content.starts_with("# H1 Title"),
    ...
    fn test_claude_arguments_json_escaping() {
        ...
        assert!(
            outputs[0].content.contains(r#""foo""#) || outputs[0].content.contains(r#"\"foo\""#)
        );
        assert!(outputs[0].content.contains("$ARGUMENTS"));
    }
    ...
    fn test_claude_validate_undocumented_placeholder() {
        ...
        assert!(
            ...
            "Should have diagnostic for undocumented $PROJECT_NAME"
        );
        assert!(diagnostics
            .iter()
            .any(|d| d.message.contains("PROJECT_NAME")));
    }
    ...
    fn test_claude_validate_documented_placeholder() {
        ...
        assert!(
            diagnostics
                .iter()
                .all(|d| !d.message.contains("PROJECT_NAME")),
            "Should not warn about documented placeholder"
        );
    }
