//! Configuration module for Calvin
//!
//! Implements configuration hierarchy per TD-7:
//! 1. CLI flags (highest priority)
//! 2. Environment variables (CALVIN_*)
//! 3. Project config (.promptpack/config.toml)
//! 4. User config (~/.config/calvin/config.toml)
//! 5. Built-in defaults (lowest priority)

mod loader;
#[cfg(test)]
mod tests;
mod types;

pub use loader::ConfigWarning;
pub use types::{
    AnimationMode, ColorMode, Config, DenyConfig, DeployConfig, DeployTargetConfig, FormatConfig,
    McpConfig, McpServerConfig, OutputConfig, SecurityConfig, SecurityMcpConfig, SecurityMode,
    SyncConfig, TargetsConfig, Verbosity,
};
