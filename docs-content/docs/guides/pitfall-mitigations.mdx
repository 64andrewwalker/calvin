---
title: Pitfall Mitigations
description: Critical architectural decisions to prevent long-term maintenance issues
---
...
```mermaid
flowchart TD
    Start[Check Target File] --> Exists{Exists?}
    Exists -- No --> Write[Write File + Lock]
    Exists -- Yes --> InLock{In Lockfile?}
    
    InLock -- No --> Manual{Manual File}
    Manual --> Skip1[SKIP]
    Manual -- --force --> Write
    
    InLock -- Yes --> HashMatch{Hash Match?}
    HashMatch -- Yes --> Safe[Safe to Overwrite] --> Write
    
    HashMatch -- No --> Modified{Modified by User!}
    Modified --> Default[Default: SKIP + WARN]
    Modified -- --force --> Force[Overwrite + WARN] --> Write
    Modified -- --yes --> Write
```
