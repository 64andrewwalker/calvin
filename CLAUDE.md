...
1. **No god objects** - Keep files under 400 lines. Large files require `calvin-no-split` justification.

2. **Domain purity** - `domain/` never imports `infrastructure/` or `application/`. It defines traits; others implement.

3. **Test first** - Add tests before changing behavior. Run `cargo test` before committing.

4. **Suggest refactoring** - If a change would benefit from restructuring, say so. Don't silently accumulate debt.

5. **Minimal changes** - Solve the problem at hand. Don't add features that weren't requested.
