...
| 退出码 | 名称 | 触发条件 | 示例 |
|--------|------|----------|------|
| `0` | SUCCESS | 命令正常完成 | `sync` 无错误 |
| `1` | ERROR | 运行时错误或验证失败 | `audit` 发现问题 |
...
| `--interactive` | CHECK_FILE_STATE | 冲突时进入 PROMPT_CONFLICT（o/s/d/a/A），可 overwrite/skip/diff/abort |
...
- name: Security Audit
  run: calvin audit --mode strict --strict-warnings
...
│           ▼                                                                 │
│   ┌───────────────┐                                                         │
│   │ FILTER_USER   │  只保留 scope=User 的资源                               │
│   │ _SCOPE_ONLY   │                                                         │
│   └───────┬───────┘                                                         │
│           │                                                                 │
│           ▼ (0 assets)                                                      │
│   ┌───────────────┐                                                         │
│   │ EMPTY_CHECK   │──────────────▶ "Nothing to install" ──▶ exit(0)        │
│   └───────┬───────┘                                                         │
│           │ (n > 0)                                                         │
│           ▼                                                                 │
│   ┌───────────────┐                                                         │
...
|-------------|------------|-----------------|
| 0.1.x | 1.0 | Claude Code 1.x, Cursor 0.44+, VS Code 1.96+, Antigravity, Codex |

---

*本规范覆盖 Calvin v0.1.0 的完整 CLI 行为*
