...
- ✅ Remote 部署支持 (通过 SyncDestination 抽象)
- ✅ 交互式冲突解决 (ConflictResolver trait)
- ✅ JSON 事件流支持 (DeployEventSink)
- ✅ post_compile 支持 (AGENTS.md 生成)
- ✅ 全面的新引擎集成测试 (8 个测试用例)
...
| 步骤 | 任务 | 优先级 | 状态 |
|------|------|--------|------|
| 1 | 添加 post_compile 到 DiffUseCase | 高 | ✅ 完成 |
| 2 | 删除旧 DeployRunner 和 bridge | 高 | ✅ 完成 (bridge 保留用于转换) |
| 3 | 合并适配器模块 | 中 | 🔄 进行中 (需删除 sync/ 后) |
| 4 | 清理 sync/ 遗留模块 | 中 | 🔲 待开始 |
| 5 | 清理 dead_code | 低 | ✅ 完成 |
| 6 | rsync 批量优化 | 低 | 🔲 待开始 |
...
|------|------|
| 2025-12-20 | 初始分析完成 |
| 2025-12-20 | post_compile 添加到 DiffUseCase |
| 2025-12-20 | Remote 部署迁移到新引擎 (SyncDestination) |
| 2025-12-20 | 交互式冲突解决迁移完成 (ConflictResolver) |
